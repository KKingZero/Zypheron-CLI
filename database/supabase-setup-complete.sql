-- COBRA AI Complete Database Setup for Supabase
...
SELECT 'Subscription history table created successfully' as status WHERE EXISTS (
  SELECT 1 FROM information_schema.tables 
  WHERE table_schema = 'public' AND table_name = 'subscription_history'
);

SELECT 'Usage logs table created successfully' as status WHERE EXISTS (
  SELECT 1 FROM information_schema.tables 
  WHERE table_schema = 'public' AND table_name = 'usage_logs'
);

-- Test the access level function
SELECT 'Testing get_user_access_level function - should return free access' as test;
SELECT * FROM public.get_user_access_level('00000000-0000-0000-0000-000000000000'::uuid);

-- Show available functions
SELECT 'Available user management functions:' as info;
...
SELECT '🎉 COBRA AI Database Setup Complete! 🎉' as result,
       'You can now sign up with shabblezam@gmail.com to get developer access' as next_step; 