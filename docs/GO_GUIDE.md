...
| `--no-tui` | Disable TUI (useful for developer/debug mode) |
| `--help`, `-h` | Show help |
...
| `analyze` | AI analysis of scan results |
| `fuzz` | Web fuzzing |
| `osint` | Open source intelligence |
| `threat` | Threat intelligence lookups |
| `bounty` | Bug bounty mode with scope filtering |
| `cloud` | Cloud security scanning (AWS, Azure, GCP, K8s) |
| `ad` | Active Directory attack module |
| `mitre` | MITRE ATT&CK objective-driven execution |
| `report` | Report generation |
| `session` | Session management (save/resume) |
...
## Autonomous Pentest (AutoPent)

```bash
# AI-driven autonomous pentest
zypheron autopent example.com

# Autonomous mode (AI decides, exploitation still requires approval)
zypheron autopent example.com --autonomous

# Save session for later
zypheron autopent example.com --save-session pentest-session.json

# Resume a saved session
zypheron autopent --resume pentest-session.json
```
...
# Authenticated scan
zypheron authenticated-scan https://example.com --token "Bearer ..."
```
...
| Category | Tools |
|----------|-------|
| **scan** | nmap, masscan, nuclei, nikto, feroxbuster, dirsearch, dirb, wpscan, whatweb |
...
| **api** | kiterunner, newman, schemathesis, jwt-tool, wfuzz |
| **recon** | subfinder, amass, theharvester, sublist3r, assetfinder |
| **password** | hydra, john, hashcat |
| **exploit** | sqlmap, metasploit, hydra, hashcat, john |
| **framework** | metasploit |
| **c2/post** | sliver, covenant, empire, bloodhound, mimikatz |
...
├── internal/
│   ├── commands/       # CLI commands (scan, recon, autopent, dork, etc.)
│   ├── tui/            # Bubble Tea TUI components
│   │   ├── views/      # TUI views (model selector, etc.)
│   │   ├── buffer/     # Output buffering
│   │   ├── scanner/    # Scan execution
│   │   ├── recovery/   # Error recovery
│   │   └── sanitize/   # Output sanitization
│   ├── tools/          # Tool execution and registry
│   ├── browser/        # Browser automation (CVE lookup)
...
Zypheron integrates with C2 frameworks (Sliver, Havoc, Metasploit) through the `exploit` command.

### Supported Frameworks

| Framework | Command | Listener Types | Output Parsing |
|-----------|---------|----------------|----------------|
| **Sliver** | `sliver` | mtls, https, http | JSON (`--json`) |
| **Havoc** | `havoc` | https, http | Text-based |
| **Metasploit** | `msfconsole` | tcp, https, http | Text-based |

### Usage

```bash
# List active C2 sessions
zypheron exploit --c2 sliver
zypheron exploit --c2 havoc

# Set up a listener
zypheron exploit --c2 sliver --listener mtls
zypheron exploit --c2 sliver --listener https
zypheron exploit --c2 havoc --listener https

# Combined with target exploitation
zypheron exploit 10.10.10.1 --c2 sliver --auto
```

### MITRE ATT&CK Coverage

C2 operations map to the following techniques:

| Technique | Name | Frameworks |
|-----------|------|------------|
| T1071 | Application Layer Protocol | Sliver, Havoc |
| T1573 | Encrypted Channel | Sliver, Havoc |
| T1105 | Ingress Tool Transfer | Sliver, Havoc |
| T1041 | Exfiltration Over C2 | Sliver, Havoc |
| T1562 | Impair Defenses | Havoc |

### Toolchain Reference

C2 tool chains are defined in `zypheron-ai/toolchains.yaml`:
- Sliver: Priority 2 (exploit), Priority 1 (red_team)
- Havoc: Priority 3 (exploit)
