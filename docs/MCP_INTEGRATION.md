...
Zypheron's MCP (Model Context Protocol) integration enables AI agents like Claude Desktop, Cursor, and VS Code Copilot to directly access Zypheron's 50+ security tools through natural language commands.
   ...
   ```json
   {
     "mcpServers": {
       "zypheron-ai": {
         "command": "python3",
         "args": [
           "/path/to/Zypheron-CLI-Production/zypheron-ai/mcp_interface/server.py",
           "--server",
           "http://localhost:8080"
         ],
...
The MCP server exposes 50+ security tools across multiple categories:
