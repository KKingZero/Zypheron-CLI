...
**Quick Help**: For common issues and troubleshooting, see [HELP.md](../HELP.md)
...
This will:
- Compile the Go binary to `build/zypheron`
- Set up Python virtual environment (if building with autopent features)
...
This copies `zypheron` to `/usr/local/bin/` making it available system-wide.

**Having issues?** See [HELP.md](../HELP.md) for troubleshooting common problems.
...
The easiest way to configure AI:

```bash
# Option 1: Secure keyring storage (recommended)
zypheron config set-key anthropic

# Option 2: Or use local Ollama (no API key needed)
curl -fsSL https://ollama.ai/install.sh | sh
ollama pull llama2
ollama serve
# Ollama will be auto-detected by Zypheron
```

### Provider Recommendations

- **Anthropic Claude** - Best for security analysis, most capable
- **Google Gemini** - Good free tier for testing
...
# Quick test
zypheron ai chat "Hello, world!"
```

**Full Details**: See [API_KEY_SETUP.md](../API_KEY_SETUP.md) for complete configuration guide including:
- All supported AI providers
- API key formats and where to get them
- Keyring vs environment variable setup
...
**Important**: Create a virtual environment to avoid system package conflicts:
...
📖 **Full MCP Guide:** See [MCP_INTEGRATION.md](MCP_INTEGRATION.md) for complete details including:
- All 30+ available security tools
- Detailed AI client configuration for Claude Desktop, Cursor, and VS Code
...
**Quick Help**: See [HELP.md](../HELP.md) for quick solutions to common problems.
