...
const AWSSecurityPanel: React.FC<{onResult?: (result: any) => void}> = ({ onResult }) => {
  ...
  const handleAssess = async () => {
    setIsRunning(true)
    try {
      const response = await fetch(getApiUrl('/api/advanced-pentest/cloud-security/assess-aws'), {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(data.error || 'AWS assessment failed')
      }
      ...
      toast.success(`AWS assessment complete. Found ${issueCount} issues`)
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error'
      toast.error(`AWS assessment failed: ${message}`)
    } finally {
      setIsRunning(false)
    }
  }
    ...
    <div className="space-y-4">
      <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-xl font-semibold">AWS Security Assessment</h3>
        </div>

        <p className="text-gray-400 text-sm mb-6">
          Comprehensive AWS security assessment covering IAM misconfigurations, S3 bucket permissions, 
          EC2 security groups, Lambda functions, RDS databases, and more (40+ checks).
        </p>
...
export default AWSSecurityPanel
