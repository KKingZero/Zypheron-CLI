...
const GCPSecurityPanel: React.FC<{onResult?: (result: any) => void}> = ({ onResult }) => {
  ...
  const handleAssess = async () => {
    setIsRunning(true)
    try {
      const response = await fetch(getApiUrl('/api/advanced-pentest/cloud-security/assess-gcp'), {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(data.error || 'GCP assessment failed')
      }
      ...
      toast.success(`GCP assessment complete. Found ${issueCount} issues`)
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error'
      toast.error(`GCP assessment failed: ${message}`)
    } finally {
      setIsRunning(false)
    }
  }
    ...
    <div className="space-y-4">
      <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-xl font-semibold">GCP Security Assessment</h3>
        </div>

        <p className="text-gray-400 text-sm mb-6">
          Comprehensive GCP security assessment covering IAM, Cloud Storage buckets, Compute Engine instances, 
          Service Accounts, Firewall rules, Cloud SQL, and more (40+ checks).
        </p>

        <button
          onClick={handleAssess}
          disabled={isRunning}
          className="w-full px-4 py-3 bg-sky-600 hover:bg-sky-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded flex items-center justify-center gap-2 font-medium transition-colors"
        >
          {isRunning ? (
            <>
...
export default GCPSecurityPanel
