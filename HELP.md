...
### Issue: ML/Exploitation Features Missing

**Symptoms:**
- ML-based analysis not available
- Exploitation tooling features not present

**Solutions:**

1. **Install optional heavy dependencies:**
   ```bash
   cd zypheron-ai
   source venv/bin/activate
   ./install-heavy.sh
...
2. **Verify optional packages:**
   ```bash
   pip show torch transformers pwntools
   ```
...
3. **Start MCP server manually:**
   ...
   cd zypheron-ai
   source mcp-venv/bin/activate
   python mcp_interface/server.py --server http://localhost:8765
   ```
   ...
   ```json
   {
     "mcpServers": {
       "zypheron-ai": {
         ...
         "args": [
           "/home/YOUR_USERNAME/Zypheron-CLI-Production/zypheron-ai/mcp_interface/server.py",
           "--server",
           "http://localhost:8765"
         ],
         "env": {
           "PYTHONPATH": "/home/YOUR_USERNAME/Zypheron-CLI-Production/zypheron-ai"
         }
       }
     }
   }
...
~/.zypheron/config.json     # Main config
