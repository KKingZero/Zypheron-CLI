...
Now with Jinja2 template support and legacy fallback
"""
...
try:
    ...
    logger.warning("Jinja2 templates not available, falling back to legacy generation")
...
class ComplianceReporter:
    ...
    def __init__(self, ai_provider=None, use_jinja=True):
        self.ai_provider = ai_provider
        self.reports: Dict[str, ComplianceReport] = {}
        self.use_jinja = use_jinja and JINJA_AVAILABLE

        if self.use_jinja:
            try:
                self.template_loader = get_template_loader()
                logger.info("Jinja2 template loader initialized")
            except Exception as e:
                logger.warning(f"Failed to initialize Jinja2: {e}, using legacy generation")
    ...
    async def _generate_executive_summary(self, report: ComplianceReport) -> None:
        ...
        try:
            # Try using prompt template if available
            if self.use_jinja and self.template_loader:
                try:
                    ...
                    logger.debug("Using Jinja2 prompt template for executive summary")
                except Exception as e:
                    logger.warning(f"Failed to load prompt template: {e}, using fallback")
                    # Fallback to inline prompt
                    prompt = self._generate_legacy_prompt(report)
            else:
                prompt = self._generate_legacy_prompt(report)
    ...
    def _generate_legacy_prompt(self, report: ComplianceReport) -> str:
        """Generate executive summary prompt (legacy inline version)"""
        return f"""
...
{"This organization demonstrates strong compliance posture." if report.compliance_percentage >= 90 else "Significant compliance gaps identified requiring immediate attention."}
    ...
    def _generate_html_report(self, report: ComplianceReport) -> str:
        """Generate HTML report using Jinja2 templates (with legacy fallback)"""
        if self.use_jinja and self.template_loader:
            try:
                context = {
                    'report': report,
                ...
                framework_template = f'compliance/{report.framework.value}.html.jinja'
                if self.template_loader.template_exists(framework_template):
                    logger.debug(f"Using framework-specific template: {framework_template}")
                    return self.template_loader.render_template(framework_template, context)

                # Fall back to generic template
                generic_template = 'compliance/base.html.jinja'
                if self.template_loader.template_exists(generic_template):
                    logger.debug("Using generic compliance template")
                    return self.template_loader.render_template(generic_template, context)

                # If templates don't exist, fall through to legacy
                logger.warning("Jinja2 templates not found, using legacy generation")

            except Exception as e:
                logger.warning(f"Jinja2 template rendering failed: {e}, using legacy generation")

        # Legacy fallback
        return self._legacy_generate_html_report(report)

    def _legacy_generate_html_report(self, report: ComplianceReport) -> str:
        """Generate HTML report (legacy string concatenation method)"""
...
"""
    ...
    def _generate_markdown_report(self, report: ComplianceReport) -> str:
        """Generate Markdown report using Jinja2 templates (with legacy fallback)"""
        if self.use_jinja and self.template_loader:
            try:
                context = {
                    'report': report,
                ...
                markdown_template = 'compliance/report.md.jinja'
                if self.template_loader.template_exists(markdown_template):
                    logger.debug("Using Jinja2 Markdown template")
                    return self.template_loader.render_template(markdown_template, context)

                logger.warning("Markdown template not found, using legacy generation")

            except Exception as e:
                logger.warning(f"Jinja2 markdown rendering failed: {e}, using legacy generation")

        # Legacy fallback
        return self._legacy_generate_markdown_report(report)

    def _legacy_generate_markdown_report(self, report: ComplianceReport) -> str:
        """Generate Markdown report (legacy string concatenation method)"""
