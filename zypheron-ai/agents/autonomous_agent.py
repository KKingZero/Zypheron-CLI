...
from core.memory import memory_manager, MemoryTier
...
class AutonomousAgent:
    ...
    def _load_persistent_memory(self):
        """Load findings from persistent session memory"""
        stored_findings = memory_manager.retrieve_context(MemoryTier.SESSION, self.task.task_id)
        if stored_findings:
            logger.info("   ↻ Loaded persistent memory for this task")
            # Logic to repopulate self.memory if needed, or just append findings
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        try:
            # Phase 0: Authorization Validation (CRITICAL!)
            logger.info("🔐 Authorization Phase: Validating permissions...")
            await self._authorization_phase()
    ...
    async def _authorization_phase(self):
        """
        Phase 0: Validate authorization before ANY action

        This is CRITICAL - no testing without explicit authorization!
        """
        from auth.authorization import get_auth_validator, AuthorizationError

        auth_validator = get_auth_validator()

        # Check if we have an authorization token
        if not hasattr(self.task, 'auth_token') or not self.task.auth_token:
            logger.critical("❌ No authorization token provided!")
            raise AuthorizationError(
                "CRITICAL: No authorization token provided!\n\n"
                "You MUST have written authorization before running autonomous agents.\n\n"
                "Steps to authorize:\n"
                "1. Obtain written authorization from system owner\n"
                "2. Create scope document: docs/scope-template.yaml\n"
                "3. Save to: ~/.zypheron/scopes/AUTH-YYYY-NNN.yaml\n"
                "4. Verify: zypheron auth list-scopes\n"
                "5. Use token when running: --auth-token AUTH-YYYY-NNN\n"
            )

        # Validate ALL targets in scope
        logger.info(f"   Validating {len(self.task.scope)} targets against authorization...")

        for target in self.task.scope:
            try:
                await auth_validator.validate_target(target, self.task.auth_token)
                logger.info(f"   ✓ {target} - AUTHORIZED")
            except AuthorizationError as e:
                logger.critical(f"   ✗ {target} - NOT AUTHORIZED")
                logger.critical(f"   Reason: {e}")
                raise AuthorizationError(
                    f"Target {target} is NOT authorized!\n\n"
                    f"Authorization Error: {e}\n\n"
                    "All targets must be explicitly authorized in your scope document.\n"
                    "See: docs/scope-template.yaml"
                )

        # Log successful authorization
        logger.info(f"✓ All {len(self.task.scope)} targets authorized")
        logger.info(f"   Authorization: {self.task.auth_token}")
        logger.info(f"   Proceeding with autonomous testing...")
        ...
        if vulns:
            ...
            for vuln in vulns:
                ...
                memory_manager.store(
                    MemoryTier.SESSION, 
                    self.task.task_id, 
                    f"finding_{len(self.memory.findings)}", 
                    finding,
                    {"severity": vuln.severity, "tool": action.tool}
                )
