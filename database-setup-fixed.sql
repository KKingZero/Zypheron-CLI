-- ========================================
-- COBRA AI - Supabase Database Setup (FIXED)
-- ========================================
...
-- ========================================
-- FIX EXISTING SUBSCRIPTION PLANS TABLE
-- ========================================

-- First, let's check and fix the subscription_plans table structure
DO $$
BEGIN
...
-- Insert/update default subscription plans (safe upsert)
...
-- ========================================
-- CREATE OTHER TABLES (Safe creation)
-- ========================================
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM auth.users WHERE email = 'shabblezam@gmail.com') THEN
    ...
    ELSE
        RAISE NOTICE 'User shabblezam@gmail.com not found in auth.users. Please sign up first, then re-run this script.';
    END IF;
END $$;
...
DROP FUNCTION IF EXISTS activate_user_subscription(UUID, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS verify_user_payment_access(TEXT);
...
BEGIN 
    ...
    RAISE NOTICE 'Developer access will be granted after shabblezam@gmail.com signs up';
