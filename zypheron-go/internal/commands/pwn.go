...
func PwnCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "pwn [binary]",
		Short: "Binary exploitation and pwnable analysis",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// SECURITY: Validate binary path for command injection prevention
			if err := validation.ValidateBinaryPath(targetBinary); err != nil {
				return fmt.Errorf("invalid binary path (potential injection): %w", err)
			}
...
// SECURITY: Uses base64 encoding for pwntools to prevent Python code injection
func buildPwnArgs(tool, binary string) []string {
	...
	case "pwntools":
		...
		// Previous vulnerable code: fmt.Sprintf("...process('%s')...", binary)
		// Attack vector: binary = "test'); malicious_code; #"
		// Now we encode the path and decode it safely in Python
		encoded := base64.StdEncoding.EncodeToString([]byte(binary))
		pythonCode := fmt.Sprintf(
			"import base64; from pwn import *; p = process(base64.b64decode('%s').decode()); p.interactive()",
			encoded,
		)
