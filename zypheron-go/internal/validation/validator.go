...
func SanitizeInput(input string) string {
	// Remove shell metacharacters including quotes and null byte
	dangerous := []string{
		";", "&", "|", "`", "$", "(", ")", "<", ">",
		"\n", "\r", "\t", "\\", "'", "\"", "\x00",
	}
...
// ValidateBinaryPath validates a binary file path for security
// Returns error if path contains dangerous characters that could enable injection
func ValidateBinaryPath(path string) error {
	...
	if len(path) > 4096 {
		return fmt.Errorf("binary path too long (max 4096 characters)")
	}

	// Check for null bytes
	if strings.Contains(path, "\x00") {
		return fmt.Errorf("binary path contains null byte")
	}

	// Check for command injection characters
	// These are critical for preventing Python/shell injection
	...
	for _, char := range injectionChars {
		if strings.Contains(path, char) {
			return fmt.Errorf("binary path contains invalid character: %q", char)
		}
	}
...
// ContainsShellMetachars reports whether input contains shell metacharacters.
// Exported for use by other packages.
func ContainsShellMetachars(input string) bool {
	return containsShellMetachars(input)
}

// containsShellMetachars checks for dangerous shell metacharacters
func containsShellMetachars(input string) bool {
	// Comprehensive list of dangerous shell metacharacters
	// Includes quotes and null byte for injection prevention
	dangerous := []string{
		";", "&", "|", "`", "$", "(", ")", "<", ">",
		"\n", "\r", "\t", "\\", "!", "~", "{", "}",
		"[", "]", "*", "?", "#", "%", "^",
		"'", "\"", "\x00", // Quotes and null byte for injection prevention
	}
