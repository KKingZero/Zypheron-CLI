...
// parseDuckDuckGoResults parses DuckDuckGo search results from HTML
func parseDuckDuckGoResults(htmlContent string) ([]SearchResult, error) {
	...
	if len(results) == 0 {
		return parseDuckDuckGoResultsRegex(htmlContent)
	}

	return results, nil
}

// parseDuckDuckGoResultsRegex uses regex as fallback for DuckDuckGo results
func parseDuckDuckGoResultsRegex(htmlContent string) ([]SearchResult, error) {
	var results []SearchResult
